...
dat$psrf = get_propensity_scores(method= "RF")

mina1 = min(dat$ps[dat$a==1])
maxa0 = max(dat$ps[dat$a==0])
mina1rf = min(dat$psrf[dat$a ==1])
maxa0rf = max(dat$psrf[dat$a==0])
...
dat_m = as.matrix(dat)
colnames_new = c()
for(i in 1:length(vars_with_imbalance))
{
  #print(i)
  ind = which(colnames(dat) == vars_with_imbalance[i])
  #print(ind)
  new_dat[,(2*i-1)] = as.numeric(dat_m[,ind])
  new_dat[,(2*i )] = (dat$a)*(as.numeric(dat_m[,ind]) - mean(new_dat[,(2*i)]))
  colnames_new = c(colnames_new, vars_with_imbalance[i], paste(vars_with_imbalance[i] , "adj", sep = "_"))
}
new_dat = new_dat[, 1:(2*length(vars_with_imbalance))]
#dim(new_dat)
colnames(new_dat) = colnames_new
new_dat$a = dat$a
new_dat$pmfu = dat$pmfu
colnames(new_dat)

lm_model1 <- lm(pmfu ~ ., data = new_dat, na.action = na.omit)
lm_model1$coefficients
new_dat=  as.data.frame(new_dat)
#summary(lm_model1)
effect <-  c(0, dim(new_dat)[1])
#lm_model2 <- lm(inclformula_1, data = dat, na.action = na.omit)
for(i in 1:dim(new_dat)[1])
{
  # cat("i is ", i)
  # cat("\n")
  temp= lm_model1$coefficients[22]
  for(j in 1:10)
  {
    temp = temp + ((lm_model1$coefficients[(2*j + 1)])*new_dat[i, ((2*j) -1)])
  }
  effect[i] = temp
}
#effect
...
for(i in 1:length(vars_with_imbalance))
{
  ...
  ind = which(colnames(dat) == vars_with_imbalance[i])
  #print(ind)
  new_dat[,i] = as.numeric(dat_m[,ind])
  #new_dat[,(2*i )] = (dat$a)*(as.numeric(dat_m[,ind]) - mean(new_dat[,(2*i)]))
  colnames_new = c(colnames_new, vars_with_imbalance[i])
}
new_dat = new_dat[, 1:(length(vars_with_imbalance))]
#dim(new_dat)
colnames(new_dat) = colnames_new
new_dat$a = dat$a
new_dat$pmfu = dat$pmfu
...
strata_dim <- rep(0,length(perc_list)+1) 
strata_var <- rep(0,length(perc_list)+1) 
strata_weight <- rep(0,length(perc_list)+1)
strata_N1 <- rep(0,length(perc_list)+1) 
strata_N0 <- rep(0,length(perc_list)+1) 

strata_dim_after <- rep(0,length(perc_list)+1) 
strata_var_after <- rep(0,length(perc_list)+1) 
strata_weight_after <- rep(0,length(perc_list)+1)

for (i in 1:(length(perc_list)+1))
{
  if (i ==1)
  {
    zst_ind <- which(dat$ps >= 0 & dat$ps <= perc_list[i] & dat$outofrange == 0)
    zst <- dat[dat$ps >= 0 & dat$ps <= perc_list[i] & dat$outofrange == 0, c("a", "pmfu")]
  } 
  else if( i == (length(perc_list)+1) )
  {
    zst_ind <- which(dat$ps >= perc_list[i-1] & dat$ps <= 1 & dat$outofrange == 0)
    zst <- dat[dat$ps >= perc_list[i-1] & dat$ps <= 1 & dat$outofrange == 0, c("a", "pmfu")]
  }
  else
  {
    #print("check")
    zst_ind <- which(dat$ps >= perc_list[i-1] & dat$ps <= perc_list[i] & dat$outofrange == 0)
    zst <- dat[dat$ps >= perc_list[i-1] & dat$ps <= perc_list[i] & dat$outofrange == 0, c("a", "pmfu")]
  }

  na_ind = which(is.na(zst[,"pmfu"]) == FALSE)
  zst = zst[na_ind,]
  N_1 <- sum(zst[,"a"]) 
  strata_N1[i] = N_1
  N_0 <- nrow(zst) - N_1 
  ...
  strata_dim[i] <- (1/N_1 * sum(Z * y) - 1/N_0 * sum((1 - Z) * y))
  V_1 <- compute_var(Z, N_1, y)
  V_0 <- compute_var(1-Z, N_0, y) 
  strata_var[i] <- V_1/N_1 + V_0/N_0
  strata_weight[i] <- nrow(zst)#/nrow(dat) 
  
  lm_sub <- lm(pmfu ~ ., data = new_dat[zst_ind,], na.action = na.omit)
  strata_dim_after[i] <- lm_sub$coefficients[12]
  strata_var_after[i] <- (summary(lm_sub)$coefficients[12,2])^2
  strata_weight_after[i] <- length(zst)/length(which(dat$outofrange == 0))
}
strata_weight = strata_weight/sum(strata_weight)
tau_dim <- sum(strata_dim * strata_weight) 
var_dim <- sum(strata_var * strata_weight) 

tau_dim_after <- sum(strata_dim_after * strata_weight) 
var_dim_after <- sum(strata_var_after * strata_weight) 

# dat$pscat[dat$outofrange==0] = as.numeric(cut(dat$ps[dat$outofrange==0], breaks=c(0,cuts,1), include.lowest=TRUE))
# dat$pscat = as.factor(dat$pscat)
# pstab = with(dat, table(a,pscat))
# pstab = rbind(pstab, with(dat, table(pscat)))
# pstab = rbind(pstab, strata_dim)
# pstab = rbind(pstab, strata_var)
# pstab = rbind(pstab, strata_N1)
# pstab = rbind(pstab, strata_N0)
print("Logistic Before and after adjusting for covariates")
#print(pstab)

res <- matrix(c(strata_N0, strata_N1, strata_N1 + strata_N0, strata_dim, strata_var, strata_dim_after, strata_var_after), ncol= 4,byrow=TRUE)
colnames(res) <- c("1", "2", "3", "4")
rownames(res) <- c("0","1","total", "DIM Before Adjusting", "Var Before adjusting", "DIM After Adjusting", "Var after adjusting")
res <- as.table(res)
res

cat("DIM estimate", round(tau_dim, 5), "\n")
cat("Variance estimate", round(var_dim,5), "\n")
cat("DIM estimate After", round(tau_dim_after, 5), "\n")
cat("Variance estimate After", round(var_dim_after,5), "\n")
 #strata_N1
```
...
dat$outofrangerf= rep(0, length(dat$outofrange))
# dat$outofrangerf[dat$psrf>maxa0 & dat$a==1]=1
# dat$outofrangerf[dat$psrf<mina1 & dat$a==0]=1
dat$outofrangerf = dat$outofrange
##### Define PS categories ##################
cuts = quantile(dat$psrf[dat$outofrangerf==0], seq(.25, .75, .25))
#dat$psrfcat[dat$outofrangerf==0] = as.numeric(cut(dat$psrf[dat$outofrangerf==0], breaks=c(0,cuts,1), include.lowest=TRUE))
#dat$psrfcat = as.factor(dat$psrfcat)
perc_list = cuts
...
for(i in 1:(length(perc_list)+1))
{
  if (i ==1)
  {
    zst_ind <- which(dat$psrf >= 0 & dat$psrf <= perc_list[i] & dat$outofrangerf == 0)
    zst <- dat[dat$psrf >= 0 & dat$psrf <= perc_list[i] & dat$outofrangerf == 0, c("a", "pmfu")]
  } 
  else if( i == (length(perc_list)+1) )
  {
    zst_ind <- which(dat$psrf >= perc_list[i-1] & dat$psrf <= 1 & dat$outofrangerf == 0)
    zst <- dat[dat$psrf >= perc_list[i-1] & dat$psrf <= 1 & dat$outofrangerf == 0, c("a", "pmfu")]
  }
  else
  {
    #print("check")
    zst_ind <- which(dat$psrf >= perc_list[i-1] & dat$psrf <= perc_list[i] & dat$outofrangerf == 0)
    zst <- dat[dat$psrf >= perc_list[i-1] & dat$psrf <= perc_list[i] & dat$outofrangerf == 0, c("a", "pmfu")]
  }
...
# pstab = rbind(pstab, strata_N1)
# pstab = rbind(pstab, strata_N0)
print("Random Forest before and after adjusting for covariates")
#print(pstab)
...
```{r, echo = FALSE}
dat$outofrange[dat$psrf>maxa0 & dat$a==1]=1
dat$outofrange[dat$psrf<mina1 & dat$a==0]=1

##### Define PS categories ##################
cuts = quantile(dat$psrf[dat$outofrange==0], seq(.25, .75, .25)) # quartiles
dat$psrfcat[dat$outofrange==0] = as.numeric(cut(dat$psrf[dat$outofrange==0], breaks=c(0,cuts,1), include.lowest=TRUE))
dat$psrfcat = as.factor(dat$psrfcat)
perc_list = cuts
...
for (i in 1:(length(perc_list)+1))
{
  if (i ==1)
  {
    ...
    zst <- dat[dat$psrf >= 0 & dat$psrf <= perc_list[i] & dat$outofrange == 0, c("a", "pmfu")]
  } 
  else if( i == (length(perc_list)+1) )
  {
    zst <- dat[dat$psrf >= perc_list[i-1] & dat$psrf <= 1 & dat$outofrange == 0, c("a", "pmfu")]
  }
  else
  {
    #print("check")
    zst <- dat[dat$psrf >= perc_list[i-1] & dat$psrf <= perc_list[i] & dat$outofrange == 0, c("a", "pmfu")]
  }
...
dat$psrfcat[dat$outofrange==0] = as.numeric(cut(dat$psrf[dat$outofrange==0], breaks=c(0,cuts,1), include.lowest=TRUE))
dat$psrfcat = as.factor(dat$psrfcat)
psrftab = with(dat, table(a,psrfcat))
psrftab = rbind(psrftab, with(dat, table(psrfcat)))
psrftab = rbind(psrftab, strata_dim)
psrftab = rbind(psrftab, strata_var)
print("Random Forest without adjusting for covariates")
print(psrftab)
...
dat$outofrange[dat$psrf>maxa0 & dat$a==1]=1
dat$outofrange[dat$psrf<mina1 & dat$a==0]=1
dat$ps = get_propensity_scores(method="logit")
dat$psrf = get_propensity_scores(method="RF")
dat$pslogit = get_propensity_scores(method="logit")
...
for (i in 1:(length(perc_list)+1))
{
  if (i ==1)
  {
    zst <- which(dat$ps >= 0 & dat$ps <= perc_list[i] & dat$outofrange == 0)
  } 
  else if( i == (length(perc_list)+1) )
  {
    zst <- which(dat$ps >= perc_list[i-1] & dat$ps <= 1 & dat$outofrange == 0)
  }
  else
  {
    #print("check")
    zst <- which(dat$ps >= perc_list[i-1] & dat$ps <= perc_list[i] & dat$outofrange == 0)
  }
  ...
  lm_sub <- lm(pmfu ~ ., data = new_dat[zst,], na.action = na.omit)
  summary(lm_model1)
  
  strata_dim[i] <- lm_sub$coefficients[12]
  strata_var[i] <- (summary(lm_sub)$coefficients[12,2])^2
  strata_weight[i] <- length(zst)/length(which(dat$outofrange == 0))
}
tau_dim <- sum(strata_dim * strata_weight) 
var_dim <- sum(strata_var * strata_weight) 

print("Logistic after adjusting for covariates")
dat$psrfcat[dat$outofrange==0] = as.numeric(cut(dat$psrf[dat$outofrange==0], breaks=c(0,cuts,1), include.lowest=TRUE))
dat$psrfcat = as.factor(dat$psrfcat)
psrftab = with(dat, table(a,psrfcat))
psrftab = rbind(psrftab, with(dat, table(psrfcat)))
...
for (i in 1:(length(perc_list)+1))
{
  if (i ==1)
  {
    zst <- which(dat$psrf >= 0 & dat$psrf <= perc_list[i] & dat$outofrange == 0)
  } 
  else if( i == (length(perc_list)+1) )
  {
    zst <- which(dat$psrf >= perc_list[i-1] & dat$psrf <= 1 & dat$outofrange == 0)
  }
  else
  {
    #print("check")
    zst <- which(dat$psrf >= perc_list[i-1] & dat$psrf <= perc_list[i] & dat$outofrange == 0)
  }
...
print("Random Forest after adjusting for covariates")
dat$psrfcat[dat$outofrange==0] = as.numeric(cut(dat$psrf[dat$outofrange==0], breaks=c(0,cuts,1), include.lowest=TRUE))
dat$psrfcat = as.factor(dat$psrfcat)
psrftab = with(dat, table(a,psrfcat))
psrftab = rbind(psrftab, with(dat, table(psrfcat)))
